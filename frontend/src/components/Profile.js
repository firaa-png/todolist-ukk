                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                    ...
                    onClick={() => {
                      ...
                      try {
                        ...
                        const mapped = parsed.map((it) =>
                          String(it.id) === String(l.id) ? { ...it, name: newName } : it
                        );
                  ...
                  <div
                    ...
                    onClick={async () => {
                      ...
                      if (isNumericId) {
                        try {
                          const res = await fetch(`${API_URL}/api/lists/${l.id}`, {
                            method: "DELETE",
                          });
                          if (!res.ok) {
                          ...
                          if (!confirm("Gagal menghapus di server. Hapus secara lokal saja?"))
                            return;
                        }
                      }
                      ...
                      try {
                        ...
                        const raw = localStorage.getItem(listsKey) || "[]";
                        const parsed = JSON.parse(raw) || [];
                        const filtered = parsed.filter((it) => String(it.id) !== String(l.id));
                        localStorage.setItem(listsKey, JSON.stringify(filtered));
                        setLists(filtered);
                        try {
                          window.dispatchEvent(
                            new CustomEvent("lists:changed", {
